<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="a in addressInfo" :key="a.id">
        <span class="username" :class="{ selected: a.isDefault == 1 }">{{
          a.consignee
        }}</span>
        <p @click="changeAddress(a, addressInfo)">
          <span class="s1">{{ a.fullAddress }}</span>
          <span class="s2">{{ a.phoneNum }}</span>
          <span class="s3" v-if="a.isDefault == 1">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>{{ orderInfo.totalNum }}</span>
      </div>
